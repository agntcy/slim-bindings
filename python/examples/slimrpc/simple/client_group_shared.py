...
async def amain(server: str, servers_str: str) -> None:
    ...
    print("SLIM_RPC_SHARED_GROUP_CLIENT_DONE", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SlimRPC shared-responses example group client")
    parser.add_argument(
        "--server",
        default=SLIM_ADDR,
    ...
    parser.add_argument(
        ...
        help="Comma-separated server instance names (must match --instance on each server_shared process)",
    )
