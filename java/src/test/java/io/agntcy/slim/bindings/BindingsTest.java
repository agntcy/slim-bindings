 ...
 * - End-to-end PointToPoint session creation, message publish/reply, and
 * cleanup
 * - Session configuration retrieval and default session configuration
 * propagation
...
class BindingsTest {
  ...
  /**
   * Validate error path when publishing to an unsubscribed / nonexistent
   * destination:
  ...
  /**
   * Test that listen_for_session times out appropriately when no session is
   * available.
   */
  @ParameterizedTest
  ...
  void testListenForSessionTimeout(String endpoint) throws Exception {
    ...
    try {
      ...
      Exception exception = assertThrows(Exception.class, () -> appAlice.listenForSession(timeout));
      ...
      assertTrue(
          (exception.getMessage() != null
              && exception.getMessage().toLowerCase(Locale.ROOT).contains("timed out"))
              || (exception.getCause() != null
                  && exception.getCause().getMessage() != null
                  && exception.getCause().getMessage().toLowerCase(Locale.ROOT).contains("timeout")));
