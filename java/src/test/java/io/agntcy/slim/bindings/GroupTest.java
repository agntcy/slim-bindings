 ...
 *
 * Scenario:
 * - A configurable number of participants join a group "chat" identified by a
 * shared topic (Name).
 * - Participant 0 (the "moderator") creates the group session, invites every
 * other participant,
 * and publishes the first message addressed to the next participant in a
 * logical ring.
...
class GroupTest {
  ...
  void testGroup(boolean mlsEnabled) throws Exception {
    ...
    try {
      ...
      System.out
          .println("[GroupTest] testId=" + testId + " mlsEnabled=" + mlsEnabled + " participants=" + participantsCount);
