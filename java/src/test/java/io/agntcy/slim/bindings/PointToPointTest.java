...
class PointToPointTest {
  ...
  void testStickySession(String endpoint, boolean mlsEnabled) throws Exception {
    ...
    try {
      ...
      int nMessages = 1000;
      System.out.println("[PointToPointTest] testId=" + testId + " endpoint=" + ep + " mlsEnabled=" + mlsEnabled
          + " nMessages=" + nMessages);
          ...
          executor.submit(() -> {
            try {
              ...
              if (server.localService() && connIdReceiver != null) {
                Name receiverNameWithId = Name.newWithId("org", "test_" + testId, "p2preceiver", receiver.id());
